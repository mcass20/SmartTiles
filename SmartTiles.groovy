...
def getThermostatData(device, type) {
	...
	def setpoint = type == "thermostatHeat" ? deviceData.heatingSetpoint : deviceData.coolingSetpoint
	setpoint = roundThermostatSetpoint(setpoint)
	[tile: "device", type: type, device: device.id, name: device.displayName, humidity: deviceData.humidity, temperature: deviceData.temperature, thermostatFanMode: deviceData.thermostatFanMode, thermostatOperatingState: deviceData.thermostatOperatingState, setpoint: setpoint, fraction: ]
}

def roundThermostatSetpoint(value) {
	def d = "$value"
	if (getTemperatureScale() == "F") {
		return d.round()
	} else {
		return 0.5 * (d / 0.5).round()
	}
}

def renderTile(data) {
	if (data.type == "thermostatHeat" || data.type == "thermostatCool") {
		return  """<div class="$data.type tile h2 thermostat ${data.setpoint ? "" : "null-setpoint"}" data-type="$data.type" data-scale="${getTemperatureScale()}" data-device="$data.device" data-setpoint="$data.setpoint" data-fraction="${setpoint % 1}"><div class="title">$data.name ${getTileIcons()[data.type]}<br/><span class="title2">${data.temperature}&deg;, $data.thermostatOperatingState</span></div><div class="icon setpoint">$data.setpoint<span class="fraction">.${setpoint % 1}</span><span class="degree">&deg;</span></div><div class="icon null-setpoint">--</div><div class="icon up"><i class="fa fa-fw fa-chevron-up"></i></div><div class="icon down"><i class="fa fa-fw fa-chevron-down"></i></div><div class="footer">&#10044; $data.thermostatFanMode ${data.humidity ? ",<i class='fa fa-fw wi wi-sprinkles'></i>" + data.humidity  + "%" : ""}</div></div>"""
